...
- [Certain programming languages](https://en.wikipedia.org/wiki/Tail_call#By_language) (mostly functional programming languages) have native support for tail recursion. If the last instruction of a function is always a recursive call, then there is no need to store the current call context information on the stack. Instead, we only replace the parameters of the call and head straight back to the beginning of the function. This optimization makes recursion a good choice for solving certain type of problems using these programming languages. One should be mindful of this, especially if you are not using functional programming languages.
